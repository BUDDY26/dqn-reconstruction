"""
scripts/run_verification.py — Phase 5 verification runner for the DQN reconstruction.
...
Usage:
    python scripts/run_verification.py --data-dir data/
    python scripts/run_verification.py --data-dir data/ --seed 7
...
Output:
    results/verification_results.json   — per-asset metrics and run metadata
    results/checkpoints/{ticker}.pt     — trained model weights per asset
"""
...
# invoked directly (e.g. python scripts/run_verification.py ...).
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Phase 5 verification runner: train and evaluate the DQN agent "
...
def _prepare_output_dirs(repo_root: Path) -> tuple[Path, Path]:
    """Create results/ and results/checkpoints/ directories if absent.
...
def main() -> None:
    ...
    print(f"Starting verification run — seed={args.seed}, assets={ASSETS}")
